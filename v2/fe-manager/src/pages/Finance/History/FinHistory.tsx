    ...
    <div>
      ...
      <Card pad={false}>
        ...
        {displayHistory.map((p, i) => {
          ...
          const nonMaxBarColor = p.total === totals.min ? '#2B7EC4' : '#00B4A0';
          const periodBarColor = p.total === totals.max ? '#B45309' : nonMaxBarColor;
          return (
            <button key={p.period} type="button" style={{ display: 'grid', gridTemplateColumns: '180px 1fr 90px 90px 140px 110px 80px', padding: '16px 24px', borderTop: i > 0 ? '1px solid #E8ECEF' : 'none', alignItems: 'center', background: i % 2 ? '#FAFBFC' : '#fff', cursor: 'pointer', transition: 'background 150ms', border: 'none', font: 'inherit', textAlign: 'left', width: '100%' }}
              onClick={() => setSel(p)}
