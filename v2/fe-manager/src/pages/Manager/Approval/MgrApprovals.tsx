...
export function ApprovalDetailDrawer({ approval: a, ctx, handled, handledBy, rejectedReason, initialRejectMode, onClose, onApprove, onReject }: Readonly<{
  ...
  const rejectedText = rejectedReason
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ position: 'fixed', inset: 0, background: 'rgba(15,25,35,0.38)', zIndex: 900, animation: 'detailBg 200ms ease', backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)', border: 'none', padding: 0, cursor: 'default' }}
      ></button>
        ...
        <div style={{ background: 'linear-gradient(145deg, rgba(30,45,61,0.92) 0%, rgba(0,90,78,0.88) 100%)', padding: '22px 24px 18px', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
          ...
          {isDone && (
            <div style={{ marginTop: 12, position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 999, ...(handled === 'approved' ? { color: '#6EE7B7', background: 'rgba(0,180,160,0.25)' } : { color: '#FCA5A5', background: 'rgba(220,38,38,0.25)' }) }}>
              {handled === 'approved' ? t('manager.approvals.detail.handledApproved') : rejectedText}
        ...
        <div style={{ background: '#f7f9fa', flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {a.kind === 'late' && ctx?.lateHistory && (
            <CtxSection title={t('manager.approvals.detail.lateHistory')}>
              ...
              {ctx.lateHistory.items.map((item) => (
                <div key={item} style={{ fontSize: 12, color: '#6B7E8E', display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
          ...
          {a.kind === 'swap' && ctx?.teamOnDay && (
            <CtxSection title={t('manager.approvals.detail.teamSchedule')}>
              {ctx.teamOnDay.map((m, i) => (
                <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', background: i % 2 === 0 ? 'rgba(200,212,220,0.1)' : 'transparent', borderRadius: 6 }}>
...
function CtxSection({ title, children }: Readonly<{ title: string; children: React.ReactNode }>) {
  return (
...
function TimeChip({ label, value, color }: Readonly<{ label: string; value: string; color: string }>) {
  return (
...
function StatBadge({ value, label, warn }: Readonly<{ value: number; label: string; warn: boolean }>) {
  return (
