...
export function buildDailyScript(
  ...
  // Priority: shift starting soon (≤ 20 min)
  const hero = shifts.find((s) => !s.clockOut);
  if (hero && !hero.clockIn) {
  ...
  if (active) {
    ...
    if (remaining > 0 && remaining <= 30) {
      return isVi ? `Còn khoảng ${remaining} phút nữa là hết ca.` : `About ${remaining} min left on your shift.`;
    }
    if (remaining <= 0) {
      return isVi ? 'Ca đã kết thúc — đừng quên chấm công ra.' : 'Shift ended — remember to clock out.';
    }
    return isVi ? `Đang trong ca tại ${active.locationName}.` : `On shift at ${active.locationName}.`;
  }
  ...
  if (upcoming) {
    ...
    if (diff <= 0) {
      return isVi ? `Ca ${startLabel} đang chờ bạn chấm công.` : `Your ${startLabel} shift is waiting — clock in.`;
    }
    return isVi
  ...
  if (doneCount > 0) {
    return isVi ? `${doneCount} ca hôm nay đã xong. Nghỉ ngơi đi.` : `${doneCount} shift${doneCount > 1 ? 's' : ''} done today. Rest up.`;
  }
